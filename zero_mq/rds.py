...
while True:
    try:
        ...
        # edg_img = cv2.GaussianBlur(source, (5, 5), 0)
        # edg_img = cv2.Canny(source, 100, 200)
        encoded, buffer = cv2.imencode('.jpg', source)
        # socket1.send("rds", base64.b64encode(buffer))
        socket1.send_multipart([b"rds", base64.b64encode(buffer)])
        # cv2.imshow("rds", source)
